...
public class BorderlessController {
    ...
    private void maximizeNormalizeIconChange() {
        MaterialDesignIconView icon = (MaterialDesignIconView) StageManager.mainWindowController.getMaximizeNormalize().getGraphic();
        if (icon.getGlyphName().equals("WINDOW_RESTORE")) {
            icon.setGlyphName("WINDOW_MAXIMIZE");
        } else {
            icon.setGlyphName("WINDOW_RESTORE");
        }
    }
